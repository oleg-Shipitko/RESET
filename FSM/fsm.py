...
class MoveToFinalPointAction(object):
    ...
    def run_action(self):
        self.expected_number_of_reached_points = number_of_reached_points + 1
        print 'goto point', self.x, self.y, self.theta, 1
        print 'Expected number of points, ', self.expected_number_of_reached_points 
        parameters = [self.x, self.y, self.theta, 1]
        stm_driver('go_to_global_point', parameters)

    def check_action(self):
        actual_number_of_reached_points = stm_driver('get_number_of_reached_points')
        print 'Number of reached points:', actual_number_of_reached_points
        if actual_number_of_reached_points == self.expected_number_of_reached_points:
            number_of_reached_points = self.expected_number_of_reached_points
            return True

class MoveToIntermediaryPointAction(object):
    def __init__(self, x, y, theta):
        self.x = x
        self.y = y
        self.theta = theta
...
class MoveToIntermediaryPointTask(object):
    def __init__(self, x, y, theta):
        self.x = x
        self.y = y
        self.theta = theta
...
class MoveToFinalPointTask(object):
    all_actions_were_completed = False
    ...
    def __init__(self, x, y, theta):
        ...
        self.theta = theta
        self.future_actions= [
            MoveToFinalPointAction(self.x, self.y, self.theta),
            SetCorrectCoordinatesAction()]
        self.future_actions.reverse()
        self.current_action = self.future_actions.pop()
...
collect_cubes_options = [{ 
        'priority': 1, 
        'tasks_list': [
            MoveToIntermediaryPointTask(0.60, 0.12, -1.57),
            MoveToFinalPointTask(0.67, 0.12, -1.57),
            MoveToFinalPointTask(0.67, 0.38, -1.57),
            MoveToFinalPointTask(0.92, 0.38, -1.57),
            MoveToFinalPointTask(0.92, 0.28, -1.57),
            TakeCubesTask(2)]},
        { 
        'priority': 2, 
        'tasks_list': [
            MoveToFinalPointTask(1.38, 0.48, -1.57),
            MoveToFinalPointTask(1.38, 0.43, -1.57),
            TakeCubesTask(1),
            MoveToFinalPointTask(1.38, 0.33, -1.57),
            TakeCubesTask(3)]},
        { 
        'priority': 3, 
        'tasks_list': [
            MoveToFinalPointTask(1.6, 0.46, -1.57),
            MoveToFinalPointTask(2.168, 0.33, -1.57)]}]
